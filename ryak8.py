...
# Глобальные переменные для блокировки
...
async def cmd_case(message: types.Message):
    # Проверка — только для чатов
    if message.chat.type not in ["group", "supergroup"]:
        return await message.answer("❌ Команду /case можно использовать только в групповых чатах!")
    ...
    case_image_url = "https://cdn.discordapp.com/attachments/1395838378859040779/1428639672438886470/raw.png?ex=6910e5a0&is=690f9420&hm=00ca0ed9f0f42ad1fedd7c8366c27726c694cd825b07ebb8adcf174ae08ce534&"
...
async def cb_open_case(callback: types.CallbackQuery):
    global case_in_progress, last_case_time

    # Проверка: если кейс уже крутится
    if case_in_progress:
        return await callback.answer("⏳ Кто-то уже крутит кейс. Подожди немного!", show_alert=True)

    # Проверка кулдауна
    if time.time() - last_case_time < 5:
        remain = 5 - int(time.time() - last_case_time)
        return await callback.answer(f"🕒 Подожди {remain} сек перед следующим открытием!", show_alert=True)

    case_in_progress = True
    last_case_time = time.time()
